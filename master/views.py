...
def Applicanttype_form(request):
    if request.method == "POST":
        applicanttypeformvalue = request.POST.get("ApplicantType", False)
        effective_date = date.today()
        ineffective_date = check_ineffective_date_present(
    ...
    if request.method == "POST":
        ...
        if Commission.objects.filter(Commissiontype=Commission_type).exists():
            ...
            newcommission = Comissionrates.objects.create(
                Commissiontype_id=commissioninst,
...
def addProductAndPolicyView(request):
    ...
    if request.method == "POST":
        ...
        multiplier_bt_cocat_type = request.POST.getlist("multiplier_bt_cocat_type")
        foir_fresh_cocat_type = request.POST.getlist("foir_fresh_cocat_type")
        foir_bt_cocat_type = request.POST.getlist("foir_bt_cocat_type")

        multiplier_fresh_min_loan_amt = request.POST.getlist(
            "multiplier_fresh_min_loan_amt"
        )
        multiplier_bt_min_loan_amt = request.POST.getlist("multiplier_bt_min_loan_amt")
        foir_fresh_min_loan_amt = request.POST.getlist("foir_fresh_min_loan_amt")
        foir_bt_min_loan_amt = request.POST.getlist("foir_bt_min_loan_amt")

        multiplier_fresh_max_loan_amt = request.POST.getlist(
            "multiplier_fresh_max_loan_amt"
        )
        multiplier_bt_max_loan_amt = request.POST.getlist("multiplier_bt_max_loan_amt")
        foir_fresh_max_loan_amt = request.POST.getlist("foir_fresh_max_loan_amt")
        foir_bt_max_loan_amt = request.POST.getlist("foir_bt_max_loan_amt")
        ...
        multiplier_fresh_number = request.POST.getlist("multiplier_fresh_number")
        multiplier_bt_number = request.POST.getlist("multiplier_bt_number")

        foir_fresh_percentage = request.POST.getlist("foir_fresh_percentage")
        foir_bt_percentage = request.POST.getlist("foir_bt_percentage")
        ...
        if product_and_policy_instance.is_valid():
            ...
            for (cocat_type, percentage, roi, min_loan_amt, max_loan_amt) in zip(
                foir_fresh_cocat_type,
                ...
                if FoirCategory.objects.filter(
                    ...
                    foir_fresh = FoirCategory.objects.filter(
                        cocat_type=cocat_type,
                        cutoff=percentage,
                        roi=roi,
                        min_loan_amt=min_loan_amt,
                    ...
                    foir_fresh = FoirCategory.objects.create(
                        cocat_type=cocat_type,
                        cutoff=percentage,
                        roi=roi,
                        min_loan_amt=min_loan_amt,
            ...
            for (cocat_type, multiplier_no, roi, min_loan_amt, max_loan_amt) in zip(
                multiplier_fresh_cocat_type,
                multiplier_fresh_number,
                multiplier_fresh_roi,
                multiplier_fresh_min_loan_amt,
                multiplier_fresh_max_loan_amt,
            ):

                if MultiplierCategory.objects.filter(
                    cocat_type=cocat_type,
                    multiplier_number=multiplier_no,
                    roi=roi,
                    min_loan_amt=min_loan_amt,
                    max_loan_amt=max_loan_amt,
                ).exists():
                    multiplier_fresh = MultiplierCategory.objects.filter(
                        cocat_type=cocat_type,
                        multiplier_number=multiplier_no,
                        roi=roi,
                        min_loan_amt=min_loan_amt,
                ...
                product_and_policy_instance.multiplier_fresh.add(multiplier_fresh)
            ...
            for (
                ...
                if FoirCategory.objects.filter(
                    cocat_type=cocat_type_bt,
                    cutoff=percentage_bt,
                    roi=roi_bt,
                    min_loan_amt=min_loan_amt_bt,
                    max_loan_amt=max_loan_amt_bt,
                ).exists():
                    foir_bt = FoirCategory.objects.filter(
                        cocat_type=cocat_type_bt,
                        cutoff=percentage_bt,
                        roi=roi_bt,
                        min_loan_amt=min_loan_amt_bt,
                    ...
                    foir_bt = FoirCategory.objects.create(
                        cocat_type=cocat_type_bt,
                        cutoff=percentage_bt,
                        roi=roi_bt,
                        min_loan_amt=min_loan_amt_bt,
            ...
            for (
                ...
                if MultiplierCategory.objects.filter(
                    cocat_type=cocat_type_bt,
                    multiplier_number=multiplier_no_bt,
                    roi=roi_bt,
                    min_loan_amt=min_loan_amt_bt,
                    max_loan_amt=max_loan_amt_bt,
                ).exists():
                    multiplier_bt = MultiplierCategory.objects.filter(
                        cocat_type=cocat_type_bt,
                        multiplier_number=multiplier_no_bt,
                        roi=roi_bt,
                        min_loan_amt=min_loan_amt_bt,
                    ...
                    multiplier_bt = MultiplierCategory.objects.create(
                        cocat_type=cocat_type_bt,
                        multiplier_number=multiplier_no_bt,
                        roi=roi_bt,
                        min_loan_amt=min_loan_amt_bt,
