...
def check_eligibility(request, id):
    ...
    for product in product_and_policy_master:
        if lead.product == product.product_name:
            store_eligibility_details[product.bank_names.bank_name] = {
                "tenure": main_applicant_personal_details.tenure.ten_type,
                "category": "Couldn't be calculated",
                "roi": "Couldn't be found",
                "loanamt": lead.loan_amt,
                "loanelig": "Couldn't be calculated",
                "loancap": "Couldn't be calculated",
                "eligibility": ELIGIBLE,
                "pro": product.processing_fee,
                "reason": "",
                "cocat_no": "Couldn't be calculated",
                "reasons": [],
            ...
            if not check_cibil_score(
                main_applicant_personal_details.cibil_score, product.cibil_score
            ):
                store_eligibility_details[product.bank_names.bank_name][
                    "reasons"
                ].append = "Less Cibil Score"
                store_eligibility_details[product.bank_names.bank_name][
                    "eligibility"
                ] = NOT_ELIGIBLE
            ...
            (
                ...
                product_max_tenure,
            ) = check_tenure_availability(
                main_applicant_personal_details.age,
                main_applicant_personal_details.retirement_age,
                main_applicant_personal_details.tenure,
                product,
            )
            ...
            if not check_salary_type(
                ...
                store_eligibility_details[product.bank_names.bank_name][
                    "reasons"
                ].append = "Salary Type must only be Bank Transfer"

            if not check_gross_salary(
                main_applicant_income_details.gross_sal,
                product.gross_min,
                product.gross_max,
            ):
                store_eligibility_details[product.bank_names.bank_name][
            ...
            if not check_net_salary(
                main_applicant_income_details.net_sal, product.net_min, product.net_max
            ):
                store_eligibility_details[product.bank_names.bank_name][
                    "eligibility"
                ] = NOT_ELIGIBLE
            ...
            if check_company_type(
                main_applicant_company_details.company_type.company_type.company_type
            ):
                store_eligibility_details[product.bank_names.bank_name][
                    "eligibility"
                ] = NOT_ELIGIBLE
            ...
            if check_designation_type(
                main_applicant_company_details.designation_type.desg_type
            ):
                store_eligibility_details[product.bank_names.bank_name][
                    "eligibility"
                ] = NOT_ELIGIBLE
            ...
            experience_info_ = check_current_and_total_experience(
                main_applicant_company_details.current_experience,
                main_applicant_company_details.total_experience,
                product.current_experience,
                product.total_experience,
            )
            if not experience_info_["eligibile"]:
                store_eligibility_details[product.bank_names.bank_name][
                    "reasons"
                ].extend(experience_info_["non_eligibility_reasons"])
                store_eligibility_details[product.bank_names.bank_name][
                    "eligibility"
                ] = NOT_ELIGIBLE

            if check_employment_type(
                main_applicant_company_details.employment_type.employment_type
            ):
                store_eligibility_details[product.bank_names.bank_name][
                    "eligibility"
                ] = NOT_ELIGIBLE
            ...
            if store_eligibility_details[product.bank_names.bank_name]["eligibility"]:
                ...
                store_eligibility_details[product.bank_names.bank_name][
                    "category"
                ] = related_bank_categories.category.cocat_type

                # /******************             Calculation of Multiplier               ****************/
                # Multiplier Fresh
                for multiplier_fresh in product.multiplier_fresh.all():
                    if (
                        multiplier_fresh.cocat_type
                        == related_bank_categories.category.cocat_type
                    ):
                        store_eligibility_details[product.bank_names.bank_name][
                            "multiplier_fresh"
                        ] = multiplier_fresh.multiplier_number
                        ...
                        store_eligibility_details[product.bank_names.bank_name][
                            "eligible_amount_fresh"
                        ] = eligible_amount
                        eligible_amount = min(
                            eligible_amount,
                            multiplier_fresh.max_loan_amt,
                            lead.loan_amt,
                        )
                ...
                for foir_fresh in product.foir_fresh.all():
                    if foir_fresh.cocat_type == related_bank_categories.cocat_type:
                        ...
                        if limit_utilized:
                            obligation_amount_fresh = (
                                limit_utilized * product.credit_card_obligation / 100
                            )
                        ...
                        pending_emi_duration_upperbound = (
                            PENDING_EMI_OBLIGATION_DURATION_UPPER_BOUND * 30
                        )
                        ...
                        cust_considerable_amount = (
                            cust_considerable_amount - obligation_amount_fresh
                        )
                        if cust_considerable_amount > 0:
                            emis = pmt(rate=rate, nper=tenure_in_months, pv=principal)
                            eligible_loan_amount = (
                                round(cust_considerable_amount / emis, 5) * 100000
                            )
                            eligible_loan_amount = min(
                                lead.loan_amt,
                                eligible_loan_amount,
                                foir_bt.max_loan_amt,
                            )
                            store_eligibility_details[product.bank_names.bank_name][
                                "foir_fresh_calculations"
                            ].append(eligible_loan_amount)
                ...
                for foir_bt in product.foir_bt.all():
                    if foir_bt.cocat_type == related_bank_categories.cocat_type:
                        ...
                        cust_considerable_amount = (
                            main_applicant_income_details.net_sal * foir_bt.cutoff / 100
                        )
    ...
    context = {"s": store_eligibility_details}
