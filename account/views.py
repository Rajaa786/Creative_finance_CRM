...
def additionaldetails(request, id):
    ...
    return render(request, 'account/Additional_Details.html', context={'applicant_type_form': applicant_type_form, 'lead_id': id, "applicants": current_additional_details, 'number_of_applicants': current_additional_details.count(), 'main_applicant': main_applicant.applicant_type if main_applicant else ""})
...
def add_applicants(request):
    ...
    if id == 0:
        add_form_1 = AdditionalDetailsForm(initial={
            "applicant_type": applicant_type, "cust_name": lead_name, "con_phone": lead_phone}, prefix="form1")
    ...
    add_form_1.fields['relation'].queryset = Relation.objects.filter(
        name='Self')
    return render(request, 'account/add_applicant.html', context={'form_1': add_form_1, 'form_2': add_form_2, 'lead_id': lead_id, 'applicant_main': main_applicant})
            ...
            if store_eligibility_details[product.bank_names.bank_name]['eligibility']:
                ...
                for multiplier_fresh in product.multiplier_fresh.all():
                    if multiplier_fresh.cocat_type == related_bank_categories.category.cocat_type:
                        ...
                        if eligible_amount:
                            store_eligibility_details[product.bank_names.bank_name]['multiplier_fresh_calculations'].append(
                                eligible_amount)
                            eligible_amount = 0
                ...
                for multiplier_bt in product.multiplier_bt.all():
                    if multiplier_bt.cocat_type == related_bank_categories.category.cocat_type:
                        ...
                        # store_eligibility_details[product.bank_names.bank_name]['loancap_fresh'] = eligible_amount
                        if eligible_amount:
                            store_eligibility_details[product.bank_names.bank_name]['multiplier_bt_calculations'].append(
                                eligible_amount)
                ...
                for foir_fresh in product.foir_fresh.all():
                    if foir_fresh.cocat_type == related_bank_categories.cocat_type:
                        ...
                        if cust_considerable_amount > 0:
                            ...
                            store_eligibility_details[product.bank_names.bank_name]['foir_fresh_calculations'].append(
                ...
                for foir_bt in product.foir_bt.all():
                    if foir_bt.cocat_type == related_bank_categories.cocat_type:
                        ...
                        if cust_considerable_amount > 0:
                            ...
                            eligible_loan_amount = min(
                                lead.loan_amt, eligible_loan_amount, foir_bt.max_loan_amt)
                            store_eligibility_details[product.bank_names.bank_name]['foir_bt_calculations'].append(
                                eligible_loan_amount)
...
def handle_form_data(request, form_1_instance, form_2_instance, id):
    ...
    if form_1_instance is not None:
        if not 'Salaried' == form_1_instance.cust_type.cust_type:
            messages.error(
                request, "Rejected. Applicants must be salaried.")
