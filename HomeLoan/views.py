...
def load_subProduct(request):
    product_id = request.GET.get('product_id')
    subProduct = SubProduct.objects.filter(product=product_id)
    return render(request, 'HomeLoan/subProduct_dropdown_list_options.html', {'subProduct': subProduct})


def eligibility(request, id):
    lead = Leads.objects.filter(pk=id).first()
    # -> collecting all applicants and co applicants
    add_dets = AdditionalDetails.objects.filter(lead_id=id)
        ...
        if add_det.cust_type == "Salaried":  # -> calculating for customer type salaried
            ...
            for bank in banks:
                ...
                if add_det.applicant_type == "Applicant":
                    prop_det = PropertyDetails.objects.filter(
                        lead_id=id).first()
                    if prop_det.prop_type == "Underconstruction and Buying From Builder" or prop_det.prop_type == "Underconstruction and Buying From Seller" or prop_det.prop_type == "Ready Possession and Buying From Builder":
                        p_type = 1
                        property_type = PropType1.objects.filter(
                            prop_det_id=prop_det.prop_det_id).first()
                    elif prop_det.prop_type == "Resale and Buying From Seller":
                        p_type = 2
                        property_type = PropType2.objects.filter(
                            prop_det_id=prop_det.prop_det_id).first()
                    else:
                        p_type = 3
                        property_type = PropType3.objects.filter(
                            prop_det_id=prop_det.prop_det_id).first()
                    ...
                    if property.p_type == 3:
                        remarks[add_det.applicant_type][bank.bank_name].append(
                            f"{property.prop_det.prop_type} is not considered by {bank.bank_name}")
                        continue

                    if not property.roomConsiderByBank():
                        remarks[add_det.applicant_type][bank.bank_name].append(
                            f"{bank.bank_name} does not consider {property.property_type.room_type} Room Type")
                        continue
                    ...
                    if property.p_type == 1:
                        ...
                        if not property.stageConsiderByBank():
                            remarks[add_det.applicant_type][bank.bank_name].append(
                                f"{bank.bank_name} does not consider {property_type.const_stage} Stage")
                            continue
                    ...
                    if property.p_type == 1 or property.p_type == 2:
                        if not property.negativeAreaConsiderByBank():
                            remarks[add_det.applicant_type][bank.bank_name].append(
                                f"{bank.bank_name} does not consider {property_type.prop.loc} Location")
                            continue

                age = Age.objects.filter(bank_id=bank.bank_id).first()
                oth = OtherDetails.objects.filter(bank_id=bank.bank_id).first()
                per_det = SalPersonalDetails.objects.filter(
                    additional_details_id__pk=add_det.pk).first()
                inc = Income.objects.filter(bank_id=bank.bank_id).first()
                inc_det = SalIncomeDetails.objects.filter(
                    addi_details_id=add_det.pk).first()
                oth_inc = SalOtherIncomes.objects.filter(
                    addi_details_id=add_det.pk).first()
                inc_foir = IncomeFoir.objects.filter(
                    bank_id=bank.bank_id).first()
                obl = Obligation.objects.filter(bank_id=bank.bank_id).first()
                exi_card = SalExistingCreditCard.objects.filter(
                    addi_details_id=add_det.pk).first()
                exi_loan = SalExistingLoanDetails.objects.filter(
                    addi_details_id=add_det.pk).first()
                oth_roi = OtherDetailsROI.objects.filter(
                    bank_id=bank.bank_id).first()
                cmp_det = SalCompanyDetails.objects.filter(
                    addi_details_id=add_det.pk).first()
                ...
                if AgeVerification.minAge(per_det, age):
                    ...
                    remarks[add_det.applicant_type][bank.bank_name].append(
                        f"Age of applicant should be greater than {age.min_age}")
                    continue

                # -> Checking Nationality
                if per_det.nationality == "Non Resident Indian":
                    remarks[add_det.applicant_type][bank.bank_name].append(
                        f"Nationality Should be Resident")
                    continue

                # -> Retire age
                retire_age, remark = AgeVerification.retireAge(
                    per_det, age, cmp_det, bank, add_det)
                remarks[add_det.applicant_type][bank.bank_name].append(remark)

                # -> Tenure
                tenure = LoanCalculation.calcTenure(
                    retire_age, per_det.age, oth.tenure)
                ...
                net_salary = 0
                gross_salary = 0
                incentives = LoanCalculation.calcIncentives(inc.incentive, inc_det.incentivesType, inc_det.incentive_amt,
                                                            inc.bonus_avg_yearly_percent, inc.bonus_avg_half_yearly_percent, inc.bonus_avg_qtr_percent, inc.incen_percent)
                bonus = LoanCalculation.calcBonus(inc_det.bonusType, inc.bonus, inc_det.bonus_amt, inc.bonus_avg_yearly_percent,
                                                  inc.bonus_avg_half_yearly_percent, inc.bonus_avg_qtr_percent, inc.incen_percent)
                rental_income = LoanCalculation.clacRentalIncome(
                    inc.rent_income, inc.rent_agreement_type, oth_inc, inc.rent_ref_in_bank, inc.rent_inc_percent)
                ...
                # -> obligation
                obligation = LoanCalculation.calcObligation(
                    obl.emi_oblig, obl.credit_card, exi_loan, exi_card, obl.emi_oblig_not_consi, obl.credit_card_oblig_percent)
                ...
                foir_percent, income_foir = LoanCalculation.calcIncomeFOIR(
                    total_income, inc_foir)
                ...
                # -> Rate Of Interest
                roi = LoanCalculation.calcROI(oth.prevailing_rate, per_det.cibil_score, oth_roi, per_det.gender, int(
                    per_det.loan_amt), per_det.cibil_type)
                if roi == -1:
                    remarks[add_det.applicant_type][bank.bank_name].append(
                        f"Your CIBIL Score is too low for approval of loan")
                    continue
    ...
    market_value = 1100
    loan_req = int(app_det.loan_amount)
    bank_eli = 0
    loan = 0
    loan_approved = {}
    ...
    #         bank_eli = LoanCalculation.calcUnderconstructionLTV(
    #             agreement_value, ltv)
    #     if p_type == 2:
    ...
    #         bank_eli = LoanCalculation.calcResaleLTV(
    #             market_value, agreement_value, stp_amt, reg_amt, ltv)
    #     print(bank_eli)
    #     if bank_eli < loan:
    #         loan = bank_eli
    ...
    #             if 'per_lac_emi' in data[key][bank.bank_name].keys():
    #                 data[app][bank.bank_name]['emi'] = round(
    #                     (loan_approved[bank.bank_name]/100000)*data[app][bank.bank_name]['per_lac_emi'], 2)
    ...
    #                 display[app][bank.bank_name]['total_income'] = round(
    #                     data[app][bank.bank_name]['total_income'], 2)
    #             if 'loan_eligibility' in data[app][bank.bank_name]:
    #                 display[app][bank.bank_name]['loan_eligibility'] = round(
    #                     data[app][bank.bank_name]['loan_eligibility']*100000, 2)
    ...
    #                 display[app][bank.bank_name]['per_lac_emi'] = round(
    #                     data[app][bank.bank_name]['per_lac_emi'], 2)
...
def PPage(request, ppid):
    if request.method == 'POST':
        ...
        bank = Bank.objects.filter(bank_id=Product.objects.filter(
...
def PPobligation(request, ppid):
    if request.method == 'POST':
        Obligation(
            ...
            credit_card_oblig_percent=int(
                request.POST['credit_card_oblig_percent']),
...
def PPProduct(request):
    if request.method == 'POST':
        ...
        Product(prod_name=product, bank_id=bank).save()
        return redirect('dashboard')
    context = {
        'banks': Bank.objects.all(),
    }
    return render(request, 'HomeLoan/Product.html', context=context)
...
def Productandpolicy_revieworedit(request, id):
    ...
    hl_income_foir_instance = HlIncomeFoir.objects.filter(
        pid=product_and_policy_instance)
    cibil_details = Cibil.objects.filter(pid=product_and_policy_instance)
    hl_other_details_instance = HlOtherDetails.objects.filter(
        pid=product_and_policy_instance)
    hl_obligations_instance = HlObligation.objects.filter(
        pid=product_and_policy_instance)
    hl_property_instance = HlProperty.objects.filter(
        pid=product_and_policy_instance)
    hl_loan_to_value_type_1_instance = HlLoan_To_Value_Type_1.objects.filter(
        pid=product_and_policy_instance)
    hl_loan_to_value_type_2_instance = HlLoan_To_Value_Type_2.objects.filter(
        pid=product_and_policy_instance)
    ...
    hl_obligations_form = HlObligationForm(
        instance=hl_obligations_instance.first())
    ...
    hl_otherdetails_form = HlOtherDetailsForm(
        instance=hl_other_details_instance.first())
    hl_income_foir_form = HlIncomeFoirForm()
    hl_loan_to_value_type_1_form = HlLoan_To_Value_Type_1Form(
        instance=hl_loan_to_value_type_1_instance.first())
    hl_loan_to_value_type_2_form = HlLoan_To_Value_Type_2Form(
        instance=hl_loan_to_value_type_2_instance.first())
    context = {
        "id": id,
...
def editbasicdetails(request):
    if request.method == 'POST':
        ...
        if hl_basicdetails_form.is_valid():
            hl_basicdetails_instances_active = HlBasicDetails.objects.filter(
                ineffective_date__gte=datetime.now(), effective_date__isnull=False)
    ...
    if request.method == 'GET':
        ...
        if action == 'edit':
            hl_basicdetails_form = HlBasicDetailsForm(
                instance=hl_basicdetails_instance)
            ...
            if hl_basicdetails_instance.ineffective_date is None:
                messages.warning(
                    request, "Basic Details Made Ineffective Please enter new Details ! ")
            else:
                messages.warning(
                    request, "Basic Details Made Ineffective Please enter new Details !  !")
            hl_basicdetails_form = HlBasicDetailsForm(
...
def editobligations(request):
    if request.method == 'POST':
        ...
        if hl_obligations_form.is_valid():
            hl_obligations_instances_active = HlObligation.objects.filter(
                ineffective_date__gte=datetime.now(), effective_date__isnull=False)
    ...
    if request.method == 'GET':
        ...
        if action == 'edit':
            hl_obligations_form = HlObligationForm(
                instance=hl_obligations_instance)
            ...
            if hl_obligations_instance.ineffective_date is None:
                messages.warning(
                    request, "Obligations Made Ineffective Please enter new Details ! ")
            else:
                messages.warning(
                    request, "Obligations Was Already Ineffective !")
            hl_obligations_form = HlObligationForm(
                instance=hl_obligations_instance)
            return render(request, 'HomeLoan/editobligationdetails.html', context={"hl_obligations_form": hl_obligations_form})
...
def editincomedetails(request):
    if request.method == 'POST':
        ...
        if hl_incomedetails_form.is_valid():
            hl_incomedetails_instances_active = HlIncome.objects.filter(
                ineffective_date__gte=datetime.now(), effective_date__isnull=False)
    ...
    if request.method == 'GET':
        ...
        if action == 'edit':
            ...
            if hl_income_instance.ineffective_date is None:
                messages.warning(
                    request, "Income Details Made Ineffective Please enter new Details ! ")
            else:
                messages.warning(
                    request, "Income Details Was Already Ineffective !")
            hl_income_form = HlIncomeForm(instance=hl_income_instance)
            return render(request, 'HomeLoan/editincomedetails.html', context={"hl_income_form": hl_income_form})
...
def editotherdetails(request):
    if request.method == 'POST':
        ...
        if hl_otherdetails_form.is_valid():
            hl_otherdetails_instances_active = HlOtherDetails.objects.filter(
                ineffective_date__gte=datetime.now(), effective_date__isnull=False)
    ...
    if request.method == 'GET':
        ...
        if action == 'edit':
            hl_otherdetails_form = HlOtherDetailsForm(
                instance=hl_otherdetails_instance)
            ...
            if hl_otherdetails_instance.ineffective_date is None:
                messages.warning(
                    request, "Other Details Made Ineffective Please enter new Details ! ")
            else:
                messages.warning(
                    request, "Other Details Was Already Ineffective !")
            hl_otherdetails_form = HlOtherDetailsForm(
                instance=hl_otherdetails_instance)
            return render(request, 'HomeLoan/editotherdetails.html', context={"hl_otherdetails_form": hl_otherdetails_form})
...
def editpropertydetails(request):
    if request.method == 'POST':
        ...
        if hl_propertydetails_form.is_valid():
            hl_propertydetails_instances_active = HlProperty.objects.filter(
                ineffective_date__gte=datetime.now(), effective_date__isnull=False)
    ...
    if request.method == 'GET':
        ...
        if action == 'edit':
            ...
            if hl_property_instance.ineffective_date is None:
                messages.warning(
                    request, "Property Details Made Ineffective Please enter new Details ! ")
            else:
                messages.warning(
                    request, "Property Details Was Already Ineffective !")
            hl_property_form = HlPropertyForm(instance=hl_property_instance)
            return render(request, 'HomeLoan/editpropertydetails.html', context={"hl_property_form": hl_property_form})
...
def editincomefoirdetails(request):
    if request.method == 'POST':
        ...
        if hl_incomeforeclosure_form.is_valid():
            hl_incomeforeclosure_instances_active = HlIncomeFoir.objects.filter(
                ineffective_date__gte=datetime.now(), effective_date__isnull=False)
    ...
    if request.method == 'GET':
        ...
        action = request.GET.get('action')
        hl_incomefoirdetails_instance = HlIncomeFoir.objects.get(
            pk=instance_id)
        if action == 'edit':
            hl_incomefoirdetails_form = HlIncomeFoirForm(
                instance=hl_incomefoirdetails_instance)
            ...
            if hl_incomefoirdetails_instance.ineffective_date is None:
                messages.warning(
                    request, "Income Foir Details Made Ineffective Please enter new Details ! ")
            else:
                messages.warning(
                    request, "Income Foir Details Was Already Ineffective !")
            hl_incomefoirdetails_form = HlIncomeFoirForm(
                instance=hl_incomefoirdetails_instance)
            return render(request, 'HomeLoan/editincomefoirdetails.html', context={"hl_incomefoirdetails_form": hl_incomefoirdetails_form})
...
def editloantovaluedetails(request):
    if request.method == 'GET':
        ...
        if type == 'type_1':
            hl_loantovalue_instance = HlLoan_To_Value_Type_1.objects.get(
                pk=instance_id)
        else:
            hl_loantovalue_instance = HlLoan_To_Value_Type_2.objects.get(
                pk=instance_id)
        if action == 'edit':
            if type == 'type_1':
                hl_loantovalue_form = HlLoan_To_Value_Type_1Form(
                    instance=hl_loantovalue_instance)
                return render(request, 'HomeLoan/editloantovaluedetails.html', context={"hl_loantovalue1_form": hl_loantovalue_form})
            else:
                hl_loantovalue_form = HlLoan_To_Value_Type_2Form(
                    instance=hl_loantovalue_instance)
            ...
            messages.warning(
                request, "Loan To Value Details Made Ineffective !")
            if type == 'type_1':
                hl_loantovalue_form = HlLoan_To_Value_Type_1Form(
                    instance=hl_loantovalue_instance)
                return render(request, 'HomeLoan/editloantovaluedetails.html', context={"hl_loantovalue1_form": hl_loantovalue_form})
...
def editcibildetails(request):
    if request.method == 'POST':
        ...
        if hl_cibil_form.is_valid():
            hl_cibil_instances_active = Cibil.objects.filter(
                ineffective_date__gte=datetime.now(), effective_date__isnull=False)
    ...
    if request.method == 'GET':
        ...
        if action == 'edit':
            ...
            messages.warning(
                request, "Cibil Details Made Ineffective Please enter new Details ! ")
            hl_cibildetails_form = CibilForm(instance=hl_cibildetails_instance)
            return render(request, 'HomeLoan/editCibil.html', context={"hl_cibildetails_form": hl_cibildetails_form})
...
def editloanvaluetype1(request):
    if request.method == 'POST':
        ...
        if hl_loantovalue1_form.is_valid():
            hl_loantovalue1_instances_active = HlLoan_To_Value_Type_1.objects.filter(
                ineffective_date__gte=datetime.now(), effective_date__isnull=False)
    ...
    if request.method == 'GET':
        ...
        if action == 'edit':
            hl_type1_form = HlLoan_To_Value_Type_1Form(
                instance=hl_type1_instance)
...
def editloanvaluetype2(request):
    if request.method == 'POST':
        ...
        if hl_loantovalue2_form.is_valid():
            hl_loantovalue2_instances_active = HlLoan_To_Value_Type_2.objects.filter(
                ineffective_date__gte=datetime.now(), effective_date__isnull=False)
    ...
    if request.method == 'GET':
        ...
        if action == 'edit':
            hl_type2_form = HlLoan_To_Value_Type_2Form(
                instance=hl_type2_instance)
